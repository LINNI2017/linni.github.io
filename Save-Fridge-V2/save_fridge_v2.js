...
(function() {
  ...
  function calcBtn() {
    ...
    id("calc-back-btn").addEventListener("click", calcBack);
    id("calc-refresh-btn").addEventListener("click", calcView);
    id("log-out").addEventListener("click", function(e) {
        window.location = ACC_URL;
        e.preventDefault();
    });
  }
  ...
  function multiURL() {
    ...
    let macroOrder = optValue("macro-order");
    let micro = optValue("multi-micro");
    let microOrder = optValue("micro-order");
    let type = "multi";
  ...
  /**
   * Display page view for back button in food calculation page,
   * display main view for back button in food calculation input page,
   * display food calculation view for back button in food calculation result page.
   */
  function calcBack() {
    if (id("calc-submit-btn").classList.contains("hidden")) {
      calcView();
    } else {
      mainView();
    }
  ...
  function calcFetch() {
    ...
    let num = id("food-calc-input").getAttribute("num");
    for (let i = 1; i <= num; i++) {
  ...
  function getResponse(response) {
    id("food-main").setAttribute("status", response.status);
   ...
   */
  function checkErr() {
    let status = id("food-main").getAttribute("status");
