...
(function() {
  ...
  const OK_STS_LOW = 200;
  // upper bound of valid status code
  const OK_STS_UPP = 300;
  ...
  function init() {
    ...
    id("food-up").addEventListener("click", upFoodView);
    id("up-food-btn").addEventListener("click", function(e) {
      e.preventDefault();
      upFoodDB();
    });
    id("back-fridge-btn").addEventListener("click", fridgeView);
    id("back-main-btn").addEventListener("click", mainView);
    id("refresh-fridge").addEventListener("click", fridgeView);
    id("refresh-up").addEventListener("click", upFoodView);
  }
  ...
  function showMess(mess) {
    ...
    if (status >= OK_STS_LOW && status < OK_STS_UPP || status === 0) {
